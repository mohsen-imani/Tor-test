...
// @iSec we added here
  uint16_t port80 = 80;
  uint16_t port443 = 443;

  if (policy->n_entries == 1){
     const short_policy_entry_t *e = &policy->entries[0];
     if (e->min_port <= port80 && port80 <= e->max_port) {
       found_match = 1;
     }
  }else if (policy->n_entries == 2){

     const short_policy_entry_t *e = &policy->entries[0];
     if ((e->min_port <= port80 && port80 <= e->max_port) || (e->min_port <= port443 && port443 <= e->max_port)) {
       found_match = 1;
     }
     e = &policy->entries[1];
     if ((e->min_port <= port80 && port80 <= e->max_port) || (e->min_port <= port443 && port443 <= e->max_port)) {
